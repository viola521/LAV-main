...
class DetLoss(nn.Module):
    def __init__(self):
        ...
        self.hm_criterion = nn.BCEWithLogitsLoss(reduction='none')#用于进行多标签分类，nn.BCEWithLogitsLoss()相当于是在nn.BCELoss()中预测结果pred的基础上先做了个sigmoid，然后继续正常算loss。
        self.ori_criterion = nn.SmoothL1Loss(reduction='none')#nn.SmoothL1Loss：计算分俩方面，当误差在 (-1,1) 上是平方损失，其他情况是L1 损失。
