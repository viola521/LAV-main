...
class BEVPlanner(nn.Module):
    def __init__(self,
        ...
        self.plan_gru = nn.GRU(4,512,batch_first=True)#Applies a multi-layer gated recurrent unit (GRU) RNN to an input sequence.
        self.plan_mlp = nn.Linear(512,2)#Applies a linear transformation to the incoming data: y=xAT+by=xAT+b
        ...
        self.cast_cmd_pred = nn.Sequential(
            ...
            nn.Sigmoid(),#Sigmoid(x)=σ(x)=1/(1+exp(−x))
        )
