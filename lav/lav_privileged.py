...
import sys
sys.path.append('/home/violayu/disk2/CARLA_0.9.14/PythonAPI/carla/dist/carla-0.9.14-py3.7-linux-x86_64.egg')
...
class LAV:
    def __init__(self, args):
        ...
        #优化器需要知道当前使用的是什么模型的参数空间。因此在训练文件中，正式开始训练前，
        # 需要把网络的参数放到优化器中。比如：optimizer = optim.Adam(model.parameters())。
        # 需要知道反向传播的梯度信息。
    ...
    def train_bev(self, bev, ego_locs, cmds, nxps, bras, locs, oris, typs, num_objs):
        ...
        #binary_cross_entropy  ln=- −wn[yn⋅logxn+(1−yn)⋅log(1−xn)]
        ...
        self.bev_optim.zero_grad()  #将模型的参数梯度初始化为0
        loss.backward()
        self.bev_optim.step() #进行单次优化
